...
namespace DMS.RabbitMQ.Consumers
{
    ...
    public class ConsumerRetryService
    {
        ...
        public ConsumerRetryService(string queueName)
        {
        ...
        public void Subscribe<T>(Action<T> handler) where T : class
        {
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                IDictionary<string, object> headers = ea.BasicProperties.Headers;//头部信息
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Info($"队列：{_queueName}，MessageId：{messageId}，第：{retryCount}次消费发生异常：", ex);
